...
* `minVolumeRank`: filter returns true if a pairs volume rank \(rankings are specific per base\) is higher than set. This filter is only useful if you want to exclude some of the top ranking volume pairs, for example set it to 5 to only allow pairs that have a volume rank of 6 or higher.
* `maxVolumeRank`: filter returns true if a pairs volume rank \(rankings are specific per base\) is lower than set. Setting it to 10, for example, would only include pairs that have a top10 volume ranking.
